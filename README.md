...
| paper-composer | 学术论文写作与构思技能。基于核心参考论文分析，结合 academic-research-skills 生成详细写作思路，按章节推进论文撰写并进行反思优化。... | 7KB | [下载](paper-composer.zip) |
| ai-deweight | Enhances academic writing originality and reduces AI detection scores using B... | 35KB | [下载](ai-deweight.zip) |
| library-researcher | 学术文献搜索下载工具。确认VPN登录后，在学校发现系统批量检索下载，智能记录成功/失败文献。 | 6KB | [下载](library-researcher.zip) |
...
| results-driven-rl | 效果驱动的强化学习实验开发。优先保证训练收敛、结果好看、图表可用于论文，不深究实现细节。当用户需要：(1) 开发RL实验代码并产出论文级结果；(2) 诊断... | 9KB | [下载](results-driven-rl.zip) |

### 📝 内容创作

| Skill | 描述 | 大小 | 下载 |
|-------|------|------|------|
| content-producer | 采访式内容生产技能（L0/L1/L2 三层架构）。 核心方法：AI 做编辑不做作者——通过采访捕捉用户原声，整理为平台适配内容。 一次采访，多平台分发。V... | 19KB | [下载](content-producer.zip) |
| diary-companion | 日记伴侣（显微镜 🔬）— 深度阅读一篇日记，通过采访式对话和三透镜思辨，与用户碰撞产生有价值的洞察。 触发词：日记分析、阅读建议、洞察提炼、思考伙伴、日记... | 8KB | [下载](diary-companion.zip) |
| vault-layer-manager | 思考花园的园丁 🔭 — 从 diary-companion 的深度对话记录中提炼 L1 洞察，与用户深度讨论选题方向。可选：全局花园梳理。 触发词：花园梳... | 7KB | [下载](vault-layer-manager.zip) |
...
| Skill | 描述 | 大小 | 下载 |
|-------|------|------|------|
| ui-ux-pro-max | UI/UX design intelligence with AI-native app support. 50 styles, 21 palettes,... | 121KB | [下载](ui-ux-pro-max.zip) |
| image-generator | Generates AI images using Gemini API with requirement confirmation workflow. ... | 12KB | [下载](image-generator.zip) |
...
| skill-creator | Guide for creating effective skills. This skill should be used when users wan... | 67KB | [下载](skill-creator.zip) |
...
| simulink-model-builder | Builds MATLAB Simulink models programmatically with cross-version compatibili... | 47KB | [下载](simulink-model-builder.zip) |
...
| Skill | 描述 | 大小 | 下载 |
|-------|------|------|------|
| github-project-manager | GitHub 项目全生命周期管理 v2：仓库初始化、README 美化、分支保护、元数据设置、质量评分、Agent 协调、定时同步、PR/Issue 管理... | 27KB | [下载](github-project-manager.zip) |
...
| req-project-dev-draft | Guides projects from requirements to development through an outcome-driven wo... | 20KB | [下载](req-project-dev-draft.zip) |
...
| autodl-deployer | AutoDL GPU云平台项目部署与迁移工具。默认交换路径：/root/autodl-fs/（上传下载都走这里，无data/子目录）。当用户需要：(1) ... | 17KB | [下载](autodl-deployer.zip) |
...
| dify-workflow-builder | Dify 智能体工作流创建与优化专家。用于：(1) 创建新的 Dify Workflow/Chatflow 应用 YAML 配置；(2) 优化现有 Dif... | 7KB | [下载](dify-workflow-builder.zip) |

### 🎬 视频

| Skill | 描述 | 大小 | 下载 |
|-------|------|------|------|
| video-wrapper | 为访谈视频添加综艺特效（花字、卡片、人物条、章节标题等）。支持 4 种视觉主题，先分析字幕内容生成建议供用户审批，再渲染视频。内置手机友好字幕增强（放大字... | 77KB | [下载](video-wrapper.zip) |
